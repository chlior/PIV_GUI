...
choice = questdlg('Do you have maskfile already?', ...
...
switch choice
    case 'yes'
        ...
        set(handles.text15,'String','mask file ready');drawnow;
    case 'no'
        choice1 = questdlg('create one','choos your answer','yes','no','no');
        switch choice1
            case 'yes'
               str1='Mark your polygon points with the left mouse button.';
               str2='Dubble press the middle button when you are finished, press';
               str3='<BACKSPACE> or <DELETE> to remove the previously selected vertex.';
...
[filename, pathname] = uiputfile( 'polymask.mat',  'Save mask file name');
...
if isstr(get(handles.edit_endTime,'string')) eframe=100000;
else
    eframe = round(etime*vidObj.FrameRate)
end
videObj.CurrentTime = stime;
% videObj.CurrentTime = stime;
sframe = round(stime*vidObj.FrameRate)
% eframe = round(etime*videObj.FrameRate)
% for k = 1 : eframe % readFrame advances automatically  
m = matfile('z.mat', 'Writable', true);
...
while k < eframe  & hasFrame(vidObj) 
    ...
    if ndims(this_frame1) == 3, this_frame1 = rgb2gray(this_frame1); end
    
%     [x,y,u,v,snr,pkh] = matpiv(handles.this_frame0,handles.this_frame1,wins,deltaT,overlap,method,[],handles.maskfile);
...
%     clear m
%      NumberOfFrames=0;                
%  str=fullfile(FolderName,sprintf('#%d.bmp',1));
% while exist(str)~= 0 
%  NumberOfFrames=NumberOfFrames+1;
%  str=fullfile(FolderName,sprintf('#%d.bmp',NumberOfFrames));
% end
%  NumberOfFrames=NumberOfFrames-1;
%  images = cell(NumberOfFrames,1);
%  
%  if NumberOfFrames>1 
%   % load the images
%   for k = 1 : NumberOfFrames  
%    str=fullfile(FolderName,sprintf('#%d.bmp',k));
%    images{k} = imread(str);
%   end
%   %write file 
%   FileName = fullfile(FolderName,VideoName);
%   writerObj = VideoWriter(strcat(FileName,'.avi'));  
%   writerObj.FrameRate =fps ;
%   open(writerObj);       % create the video file 
